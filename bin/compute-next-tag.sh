...
#
# Tags look like `v<Owncast version>-<release>`:
#
# - if defaults/main.yml points at an Owncast version that has never been
#   released, the release counter restarts at 0 (`v0.2.5-0`)
# - otherwise the counter is incremented (`v0.2.5-1`), but only if something
...
version="$(sed -nE 's|^owncast_version:[[:space:]]*"?([^"[:space:]]+)"?.*$|\1|p' "$defaults_path" | head -n1)"

if [ -z "$version" ]; then
	echo >&2 "Could not determine the Owncast version from $defaults_path"
	exit 1
fi

# The version values carry no leading `v` (e.g. `0.2.5`), but the tags do.
# Stripping a `v` that is not there is harmless and keeps this correct
# should the value ever gain one.
